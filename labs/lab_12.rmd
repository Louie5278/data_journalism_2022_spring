...
```{r}
police_incidents_by_date <- read_csv("https://raw.githubusercontent.com/dwillis/wvu-projects/master/crime-log/crime_log.csv")
  
police_incidents_by_date <- police_incidents_by_date %>%
  mutate(datetime = mdy_hm(datetime)) %>% 
  mutate(date = date(datetime), hour = hour(datetime)) %>% 
  mutate(day_of_week = wday(date))
  
most_recent<-police_incidents_by_date %>% 
  arrange(desc(date)) %>% 
  slice(1)
...
**A2.**
Maryland's lowest zip code is 20601 in Waldorf and the highest is 21930 in Germantown 
```{r}

vaccinations_by_zipcode <- read_csv("https://raw.githubusercontent.com/state-of-maryland/VaccineCSVs/master/Vax_ZIP_CODE_FirstSecondSingle_Totals_Raw.csv")

vaccinations_by_zipcode <- vaccinations_by_zipcode %>% 
 mutate(date=today()) %>% 
