...
https://datawrapper.dwcdn.net/QTitl/4/
...
It is grouping all of the counties in MD and counting the amount of loans in each county
```{r}
...
**Answer** Describe what the code below is doing and the output.
Filtering for loans that dont appear to be MD loans 
```{r}
md_counties <- maryland_ppp %>%
    group_by(project_county_name) %>%
    summarise(count=n()) %>%
...
It created a 5 digit fips code 
```{r}
...
removed the county and replacing some names with correct titles so we can join the data and now it is together in one place 
```{r}
...
Now we are only looking at MD and its counties
...
**Answer** Describe the changes from the previous map
Now everything is capitalized correctly and we can can see county names and the numbers have commas 
...
https://datawrapper.dwcdn.net/qSx6P/2/