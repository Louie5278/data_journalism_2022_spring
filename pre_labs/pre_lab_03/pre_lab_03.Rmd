...
**Answer** amount is type <dbl> and name is <chr> id is also <dbl> 
```{r}
...
**Answer**:
There are 192959 franchises with NA names. 275 are subway. 
```{r}
...
**Answer**:
yes, we can conclude that means there are no discrepencies between the dates forgiven and amount forgiven. If they were not the same that would mean that something in the data is off and we would have to investigate why the numbers match up. 
```{r}
ppp_maryland_loans %>%
...
**Answer** Max amount is 10000000	and the minimum is 6. This is suspicious to me because I thouhgt the minimum loan amount was around $30k
```{r}
...
**Answer** The city is spelled differntly multiple times, There is a duplicate in the data and there is a space between street and address in one of the columns. 
...
**Answer**
It says that there is an invalid type of argument, I think we cannot add up all of the loans because the data in the amount column is not the right type so it cannot be recognized. 
```{r}
# cleaning function
...
**Answer**
There is a duplicate for Stuart Jones LLC
```{r}
cleaned_arnold_md_loans %>%
  get_dupes()
```
...
**Answer**
This standardizes all of the zip codes so that they are all 5 digits so the data is more uniform 
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
