...
**Answer** I see state names with data about voting patterns in the 2016 presidential election.  
```{r}
...
**Answer** I see a graph that shows the amount of businesses receiving ppp loans in relation to the percent of people that voted for trump
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** This shows a graph that shows a positive relationship between the employees receiving ppp loans and those who voted for trump but it is not at reliable or revealing as the previous graphs 
```{r}
...
**Answer** we see the direct correlation between the percent of people who voted for trump in relation to the businesses and employees getting ppp loans and the businesses are significant because its over .5 but the individuals are less significant because it is only .2 
```{r}
reveal_data %>%
...
**Answer** The model output shows us the pearson's product moment correlation with t values, p values and confidence intervals and it is over .5 so it is significant relation 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer**  This shows the relationship between the pct_trump and the ppe_amount_per_employe and it is not significant
```{r}
...
**Answer** Now we are comparing the presidential candidates and the relations between who voted for who and who was getting ppp loans and we can see there is a bigger relation to Trump states/supporters
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** This shows that both candidates have suspicious amounts of ppp loans going out to their supporters
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** This shows that the data points towards Trump giving out ppp loans to the states that supports them 
```{r}
