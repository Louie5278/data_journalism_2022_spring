...
**Answer**
Now it is highlighting Dog and Cat and dog and cat tweets and cleaning up the cases where there are duplicates 
```{r}
...
**Answer**
A database of all of the unique words from our original dataframe 
```{r}
...
**Answer**
It shows the amount of times the top 25 unique words appear in our dataframe in a bar graph. Most of the words are generic words like "the" or "and"
...
**Answer**
Now we have removed the stop words and now we have the top 10 most unique words that are more meaningful
```{r}
unique_words %>%
...
**Answer**
You can see patterns in tweet patterns and compare them by year 
```{r}
unique_words_2020 %>%
  anti_join(stop_words) %>%
  group_by(word) %>%
...
**Task** Run the following code and describe the results.
**Answer**
We see that the top words from 2020 are health and health care besides covid19
```{r}
covid_tweets %>%
  filter(created < '2021-01-01') %>%
...
**Task** Run the following code and describe the results.
**Answer**
Now the most common phrases are those with the word vaccine in it 
```{r}
covid_tweets %>%
  filter(created >= '2021-01-01') %>%
  unnest_tokens(bigram, content, token = "ngrams", n = 2) %>%
...
**Task** Run the following code and describe the results.
**Answer**
democrats tweeted more about health care while republicans talked more about the pandemic
```{r}
covid_tweets %>%
...
**Task** Run the following code and describe the results.
**Answer**
We can see that the parties were using different terms in different ways. Framing the pandemic differently by each political party
```{r}
covid_tweets %>%
...
**Answer**
Some things may be incorrect such as Trump being positive but for the most part the programs seems to get the connotations correct 
```{r}
