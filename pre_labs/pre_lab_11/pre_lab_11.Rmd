...
**Answer**
I see a list of hospital files from feb 4 2020
```{r, echo=FALSE}
...
**Answer**
there are 33 columns and in this data I see a collection of hospital files. some column examples are name, telephone and population 
```{r}
...
**Answer**
I see a list of MD hospitals with their id's name address city state and zip
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
There is a list of types of hospitals and their type, such as psychiatric, rehab or military 
```{r}
md_hospitals %>%
  group_by(TYPE) %>%
  summarise(
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
I see a list of the 50 general acute care hospitals and data about them
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Now we have a scatter plot of the longitude and latitude of the hospitals
```{r}
md_hospitals %>%
  ggplot() + 
  geom_sf() + 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** I see that it installed and ran
```{r}
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** I see a data set of counties with 3,234 rows
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Now we only have maryland counties
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We see a longitude and latitude graph of MD and broken into counties 
```{r}
md_counties %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We now plotted the hospitals where they are in each MD county 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Now the dots are color coded to show which ones have the most to least amount of beds 
```{r}
...
**Answer**
The libraries were installed and ran
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We have a dataframe of hospital info but its filtered for the 50 genreal acute care hospitals in MD
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We have each MD county and the count for amount of total beds in descending order 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We have MD counties with population info
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Output of the top 10 most populous counties
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We see a list of the counties in all capital 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We now have cleaned the population data and changed the name column to "county"  
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Joined the data now that it is clean
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We are changing the values with no hospitals from NA to the numerical value 0 
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
a list of hospitals in MD counties with the number of beds per 100,000 for each county and sort from highest to lowest
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
  mutate(total_beds = replace_na(total_beds,0)) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
now we have a longitude and latitude graph with MD counites and the hospitals are color coded to show which counties have hospitals with the most beds to the least 
```{r}
ggplot() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Now the graph is color coded but there are multiple colors
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
...
**Answer**
Now that we have changed it to a log scale we can see the colors diffrence a lot better. I see a pattern that some of the counties that have a smaller population have more beds per 100,000 people than the bigger counties
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
